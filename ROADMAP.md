...
- [ ] **TUI scheduler.** `nevinho chat` does not start a runner today, so schedules created from the terminal never fire. Wire `startScheduler` into `cmd/chat.go`, stopped cleanly on exit. Delivery is pull-first, not push-first:
  - `/schedules show <name>` renders the most recent run from `Runs[]`. Optional `--run N` for older entries. This is the must-have, makes scheduled output visible whenever the user opens the terminal.
  - When the TUI is open at fire time, render the result inline as a system-attributed message (distinct styling, dim color). The schedule's history stays isolated under `scheduler:<id>`; the inline render is presentation only, not a turn the model has to track.
  - One-line awareness footer at the bottom of `/status` and after `/schedules`: `3 schedules active · last fire: hn_daily 2h ago`. Informational, not interrupting.
  - Skip: launch banners counting missed fires, OS notifications, replaying old runs into chat history. All push patterns that fight the coding-assistant mental model.
